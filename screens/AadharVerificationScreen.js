...
export default function AadharVerificationScreen({ navigation, route }) {
  ...
  const handleVerifyOtp = () => {
    if (validateOtp()) {
      // 🔴 Demo OTP (replace with real API later)
      if (otp === '123456') {
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              value={aadharNumber}
              onChangeText={(text) => {
                setAadharNumber(formatAadharNumber(text));
...
const styles = StyleSheet.create({
  ...
  sendOtpButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  verifyButton: {
    backgroundColor: '#10b981',
    padding: 16,
    borderRadius: 12,
  ...
  resendLink: { color: '#10b981', fontWeight: '600' },
  noteContainer: {
    marginTop: 24,
    backgroundColor: '#f0fdf4',
    padding: 12,
  ...
  noteText: { textAlign: 'center', color: '#065f46', fontSize: 12 },
